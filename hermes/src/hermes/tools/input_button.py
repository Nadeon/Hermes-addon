"""Hermes — Tools MCP para input_button de Home Assistant."""
...
def _dump_config(model: InputButtonConfig) -> dict[str, Any]:
    return model.model_dump(exclude_none=True, by_alias=True, mode="json")


def register(mcp: object, ha_client: HAClient) -> None:
    """Registra las tools de input_button."""

    ready = requires_ready(ha_client)

    @mcp.tool()
    @ready
    async def ha_list_input_buttons() -> object:
        """Lista los input_button disponibles."""
    ...
    @mcp.tool()
    @ready
    async def ha_get_input_button(entity_id: str) -> object:
        """Devuelve la config persistida y el estado de un input_button."""
        try:
    ...
    @mcp.tool()
    @ready
    async def ha_create_or_update_input_button(
        entity_id: str,
        config: dict[str, Any],
        confirmation_token: str | None = None,
    ) -> object:
        """Crea o modifica un input_button vía WS. Preview + token."""
        try:
            normalized_config = _dump_config(
                InputButtonConfig.model_validate(config)
            )
        except Exception as exc:  # noqa: BLE001
            logger.warning(
                "ha_create_or_update_input_button_validation_failed",
                entity_id=entity_id,
                error=str(exc),
            )
    ...
    @mcp.tool()
    @ready
    async def ha_delete_input_button(
        entity_id: str,
        confirmation_token: str | None = None,
    ) -> object:
        """Elimina un input_button vía WS. Siempre requiere `confirmation_token`."""
    ...
    @mcp.tool()
    @ready
    async def ha_press_input_button(entity_id: str) -> object:
        """Emite un pulso en un input_button. Mapea a `input_button.press`."""
        eid = _entity_id_of(entity_id)
        if not await entity_exists(ha_client, eid):
            logger.warning("ha_press_input_button_not_found", entity_id=eid)
            return not_found_response(eid)
        return await ha_client.call_service(DOMAIN, "press", {"entity_id": eid})
