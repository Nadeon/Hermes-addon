"""Hermes — Tools MCP para input_text de Home Assistant."""
...
def _dump_config(model: InputTextConfig) -> dict[str, Any]:
    return model.model_dump(exclude_none=True, by_alias=True, mode="json")


def register(mcp: object, ha_client: HAClient) -> None:
    """Registra las tools de input_text."""

    ready = requires_ready(ha_client)

    @mcp.tool()
    @ready
    async def ha_list_input_texts() -> object:
        """Lista los input_text disponibles."""
    ...
    @mcp.tool()
    @ready
    async def ha_get_input_text(entity_id: str) -> object:
        """Devuelve la config persistida y el estado de un input_text."""
        try:
    ...
    @mcp.tool()
    @ready
    async def ha_create_or_update_input_text(
        entity_id: str,
        config: dict[str, Any],
        confirmation_token: str | None = None,
    ) -> object:
        """Crea o modifica un input_text vía WS. Preview + token."""
        try:
            normalized_config = _dump_config(
                InputTextConfig.model_validate(config)
            )
        except Exception as exc:  # noqa: BLE001
            logger.warning(
                "ha_create_or_update_input_text_validation_failed",
                entity_id=entity_id,
                error=str(exc),
            )
    ...
    @mcp.tool()
    @ready
    async def ha_delete_input_text(
        entity_id: str,
        confirmation_token: str | None = None,
    ) -> object:
        """Elimina un input_text vía WS. Siempre requiere `confirmation_token`."""
    ...
    @mcp.tool()
    @ready
    async def ha_set_input_text(entity_id: str, value: str) -> object:
        """Pone un input_text a un valor concreto. Mapea a `input_text.set_value`."""
        eid = _entity_id_of(entity_id)
        if not await entity_exists(ha_client, eid):
            logger.warning("ha_set_input_text_not_found", entity_id=eid)
